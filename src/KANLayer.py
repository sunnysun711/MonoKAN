...
from src.spline_hermite import *
...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0,
                base_fun=torch.nn.ELU(), grid_eps=0.02, grid_range=[-1, 1], hermite=False, sp_trainable=True, sb_trainable=True, device='cpu'):
        ''''
        initialize a KANLayer
        ...
        -----
            ...
            base_fun : function
                residual function b(x). Default: torch.nn.ELU()
            grid_eps : float
        ...
        if not hermite:
            ...
            self.coef = torch.nn.Parameter(curve2coef_hermite(self.grid, noises, self.grid, device))
